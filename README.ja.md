...
[製品ページ](https://wubing2023.github.io/PaperSpine/v5/) · [GitHub Release](https://github.com/WUBING2023/PaperSpine/releases/tag/v0.4.0-alpha.1-dev)
...
- Windows x64 suite：約 26.4 MB。
- Linux glibc x86_64 suite：約 56.2 MB。
- macOS Apple Silicon suite：約 40.5 MB。
- macOS Intel x86_64 suite：約 40.5 MB。
- 単体の `paper-spine` Skill：既存のホスト実行環境がある場合向け。約 0.72 MB。

すべてのダウンロードは公開 manifest と SHA-256 に紐づきます。現在のバージョンは `v0.4.0-alpha.1-dev` プレリリースです。
...
`-CleanLegacy` は既知の V3/V4 Skill 検出ディレクトリを退避するだけで、論文タスクのデータ、ホスト設定、未知のファイルは削除しません。どちらのインストーラーもバイト数、SHA-256、suite 内部の整合性、初回起動時のヘルスチェックを検証します。
...
profile が存在する場合、2 番目のコマンドはトランザクション方式の `update` として動作し、タスクデータを保持したまま初回起動処理を実行します。自動更新は既定で無効です。
...
- macOS パッケージは署名も notarize もされていないため、初回起動時にユーザーの明示的な許可が必要になる場合があります。
