...
def check_matrix(matrix_path: Path, manuscript_text: str, lang: str, humanize_tier: str = "medium", thresholds: HumanizeThresholds | None = None, threshold_warnings: list[str] | None = None) -> HumanizeCheckResult:
    ...
    # Store threshold snapshot in result
    result.thresholds = {
    ...
    if result.matrix_rows > 2 and severity_counts["high"] == 0:
        structural.append("No high-severity patterns found — matrix may be under-reporting")

    missing_dims = set(CNKI_DIMENSIONS) - dim_hits
    if missing_dims:
        structural.append(f"Dimensions not covered: {', '.join(sorted(missing_dims))}")
    ...
    for dimension in result.dimension_results.values():
        if dimension.status != "PASS":
            if dimension.code in required_set:
                required_dim.extend(dimension.findings)
            else:
                advisory_dim.extend(dimension.findings)

    result.required_findings = dedupe(structural + required_dim)
    result.advisory_findings = dedupe(advisory_dim)
    result.findings = dedupe(structural + required_dim + advisory_dim)
