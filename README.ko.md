...
[제품 페이지](https://wubing2023.github.io/PaperSpine/v5/) · [GitHub Release](https://github.com/WUBING2023/PaperSpine/releases/tag/v0.4.0-alpha.1-dev)
...
모든 다운로드는 공개 manifest와 SHA-256으로 관리됩니다. 현재 버전은 `v0.4.0-alpha.1-dev` 프리릴리스입니다.
...
`-CleanLegacy`는 알려진 V3/V4 Skill 검색 폴더를 백업 위치로 옮길 뿐이며, 논문 작업 데이터와 호스트 설정, 알 수 없는 파일은 삭제하지 않습니다. 두 설치 프로그램 모두 바이트 수, SHA-256, suite 내부 무결성, 첫 실행 상태 점검을 확인합니다.
...
profile이 있으면 두 번째 명령은 트랜잭션 방식 `update`로 동작하며, 작업 데이터를 유지하고 첫 실행 검사를 수행합니다. 자동 업데이트는 기본적으로 꺼져 있습니다.
...
- macOS 패키지는 서명되지 않았고 notarize도 거치지 않아, 첫 실행 시 사용자의 명시적 허용이 필요할 수 있습니다.
