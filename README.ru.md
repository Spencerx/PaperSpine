...
<p align="center"><img src="website/assets/brand/paperspine-figure-editable.webp" alt="Сверху: полная научная схема, которую выдаёт PaperSpine5. Снизу: та же схема после Ctrl+A в PowerPoint — все 122 элемента остаются редактируемыми нативными объектами."></p>

**Иллюстрации — не вставленные скриншоты.** Полученные научные иллюстрации — это редактируемые файлы PPTX: нажмите Ctrl+A на этом примере в PowerPoint, и выделятся 122 нативных объекта; текст, стрелки и кривые данных остаются редактируемыми.
...
- Отдельный Skill `paper-spine`: для уже установленной среды хоста, около 0,72 МБ.
...
Если установка уже есть, вторая команда выполняет обновление с возможностью отката, сохраняет данные задач и запускает самопроверку. Автоматические обновления по умолчанию отключены.
