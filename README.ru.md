...
Если установка уже есть, вторая команда выполняет обновление с возможностью отката, сохраняет данные задач и запускает самопроверку. Каждый вызов Skill проверяет обновления и при необходимости обновляет suite и сам инструмент обновления; явное отключение автообновления учитывается.
