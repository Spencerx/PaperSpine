...
Все загрузки привязаны к публичному manifest и SHA-256. Текущая версия — `v0.4.0-alpha.1-dev` (предрелиз).
...
`-CleanLegacy` только архивирует известные каталоги обнаружения Skill версий V3/V4; данные задач, настройки хоста и неизвестные файлы не удаляются. Оба установщика проверяют размер в байтах, SHA-256, внутреннюю целостность suite и состояние при первом запуске.
...
Если профиль уже существует, вторая команда выполняется как транзакционное `update`, сохраняет данные задач и выполняет первичный старт. Автоматические обновления по умолчанию отключены.
